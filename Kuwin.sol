...
contract ContractDataLayout is LibraryLock {
    ...
    string public _name = "KuWin";
    string public _symbol = "KUWIN";
    ...
    uint256 kuwinDrawPrize = 8 * 10**18;
    ...
    IUniswapV2Router02 public koffeeRouter;
    address public koffeePair;
...
contract ERCStorage {
    ...
    function sendToken1() public {
        require(msg.sender == kuWin);
        IERC20(token1).transfer(kuWin, IERC20(token1).balanceOf(address(this)));
    }
...
contract KuWin is Context, IERC20, Ownable, ContractDataLayout, Proxiable {
    ...
    function proxyConstruct() public {
        ...
        token1 = 0x4446Fc4eb47f2f6586f9fAAb68B3498F86C07521; //set address
        IUniswapV2Router02 _koffeeRouter = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //set address
         // Create a uniswap pair for this new token
        koffeePair = IUniswapV2Factory(_koffeeRouter.factory())
            .createPair(address(this), token1);

        // set the rest of the contract variables
        koffeeRouter = _koffeeRouter;
    ...
    function calculatePoolWithdrawal(address winner) internal returns(uint256) {
        //get amount of max daily % from pool
        uint256 poolToken1Amount = IERC20(token1).balanceOf(koffeePair);
    ...
    function removeLiquidity(uint256 liquidity) internal {
        ...
        try koffeeRouter.removeLiquidity(
            address(this),
            token1,
            liquidity,
        ...
        //burn token0 
        _tokenTransfer(address(this), address(1), balanceOf(address(this)).sub(currentToken0Bal), false);
    }
    ...
    function lotteryCheck() internal {
        ...
        //check if liquidity WKCS is equal to 88 or more
        //Avoid transferFrom issues
        if (IERC20(koffeePair).balanceOf(address(0)) == 0) {
            return;
        }
        ...
        if (winnerValid) {
            ...
            //dynamic lottery amount calculation
            uint256 sentAmount = calculatePoolWithdrawal(idAddress[winningUser]);
            _winningUsers.push(idAddress[winningUser]);
            _winningAmount.push(sentAmount);
            totalToken1Given = totalToken1Given.add(sentAmount);
            totalWinners = totalWinners.add(1);
         
            trysUntilDraw.push(currentTrys);
            currentTrys = 0;
            emit WinnerSelected(idAddress[winningUser], sentAmount);
    ...
    function getRandomNumber(uint256 upperNumber, bool shuffle) internal view returns(uint256) {
        ...
        uint256 wrappedKCSBalance = address(0x4446Fc4eb47f2f6586f9fAAb68B3498F86C07521).balance; //need KCS address
        uint256 shuffler;
        if (shuffle) {
            shuffler = wrappedKCSBalance.div(3).add(block.timestamp.div(2));
        }
        uint256 randomResult = uint256(keccak256(abi.encodePacked(
            wrappedKCSBalance + block.timestamp + block.difficulty +
    ...
    function _setUserID(address user) internal {
        if (user == koffeePair || user == address(this) || user == address(1) || user == address(0)) {
            return;
        }
        if (userId[user] == 0) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        swapTokensForToken1(half); // <- this breaks the WKCS -> HATE swap when swap+liquify is triggered
        ERCStorage(tokenStorage).sendToken1();
        ...
        if (newBalance > 2) {
            ...
            // IERC20(WKCS).transfer(marketingWallet, newBalance.mul(20).div(100));
        
            //determine value of 1/3 of WKCS tokens in Lucky
            //if WKCS value is higher, use all lucky in contract
    ...
    function swapTokensForToken1(uint256 tokenAmount) private {
        ...
        _approve(address(this), address(koffeeRouter), tokenAmount);

        // make the swap
        try koffeeRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 token1Amount) private {
        ...
        _approve(address(this), address(koffeeRouter), tokenAmount);
        IERC20(token1).approve(address(koffeeRouter), token1Amount);

        // add the liquidity
        try koffeeRouter.addLiquidity(
            address(this),
            token1,
            tokenAmount,
            token1Amount,
