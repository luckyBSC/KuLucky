...
contract ContractDataLayout is LibraryLock {
    ...
    string public _name = "KuLucky";
    string public _symbol = "KULUCKY";
    ...
    uint256 luckyDrawPrize = 8 * 10**18;
    ...
    struct userSwapStruct {
        uint256 WKCSProvided;
        bool LPClaimed;
    }
    mapping(address => userSwapStruct) public userSwap;
    uint256 public totalWKCSProvided;
    uint256 public LPCreated;
}

contract ERCStorage {
    address lucky;
    address WKCS = 0x4446Fc4eb47f2f6586f9fAAb68B3498F86C07521;
    constructor () public {
        lucky = msg.sender;
    }
    
    function sendWKCS() public {
        require(msg.sender == lucky);
        IERC20(WKCS).transfer(lucky, IERC20(WKCS).balanceOf(address(this)));
    }
...
contract LuckV3 is Context, IERC20, Ownable, ContractDataLayout, Proxiable {
    ...
    function tokenSwap(uint256 amount, address token) public {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            amount,
            0, // accept any amount of WKCS
            path,
            address(this),
            block.timestamp
        );
        uint256 postBal = IERC20(token).balanceOf(address(this));
        userSwap[msg.sender].WKCSProvided = userSwap[msg.sender].WKCSProvided.add(postBal.sub(preBal));
        totalWKCSProvided = totalWKCSProvided.add(postBal.sub(preBal));
        //deposit event
    }

    function initializeLiquidity() public onlyOwner {
        IERC20(WKCS).approve(address(uniswapV2Router), IERC20(WKCS).balanceOf(address(this)));
        IERC20(address(this)).approve(address(uniswapV2Router), IERC20(address(this)).balanceOf(address(this)));

        uniswapV2Router.addLiquidity(
            WKCS,
            address(this),
            IERC20(WKCS).balanceOf(address(this)),
    ...
    function claimLP() public {
        ...
        uint256 claimable = userSwap[msg.sender].WKCSProvided.mul(LPCreated).div(totalWKCSProvided);
    ...
    function lotteryCheck() internal {
        ...
        if (winnerValid) {
            if (IERC20(WKCS).balanceOf(address(this)) < luckyDrawPrize) {
                return;
            }
            previousWinner = winningUser;
            previousWinningBlock = block.number;
            IERC20(WKCS).transfer(idAddress[winningUser], luckyDrawPrize);
            _winningUsers.push(idAddress[winningUser]);
            _winningAmount.push(luckyDrawPrize);
            totalWKCSGiven = totalWKCSGiven.add(luckyDrawPrize);
            totalWinners = totalWinners.add(1);
            jackpotAmount = IERC20(WKCS).balanceOf(address(this));
            luckyDrawAmount = 0;
    ...
    function jackpotCheck() internal {
        ...
        if (jackpotAmount >= jackpotPrize) {
            ...
            if (winnerValid) {
                previousWinner = winningUser;
                previousWinningBlock = block.number;
                IERC20(WKCS).transfer(idAddress[winningUser], jackpotAmount);
                _winningUsers.push(idAddress[winningUser]);
                _winningAmount.push(jackpotAmount);
                totalWKCSGiven = totalWKCSGiven.add(jackpotAmount);
                totalWinners = totalWinners.add(1);
                jackpotAmount = 0;
    ...
    function _transfer(
        ...
        ERCStorage(tokenStorage).sendWKCS();
    ...
    function manualDrawAdd(bool normal, uint256 amount) public {
        IERC20(WKCS).transferFrom(msg.sender, address(this), amount);
        if (normal) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 initialBalance = IERC20(WKCS).balanceOf(address(this));
        // swap tokens for WKCS
        swapTokensForWKCS(half); // <- this breaks the WKCS -> HATE swap when swap+liquify is triggered
        ERCStorage(tokenStorage).sendWKCS();
        
        // how much WKCS did we just swap into?
        if (IERC20(WKCS).balanceOf(address(this)) <= initialBalance) {
            return;
        }
        
        uint256 newBalance = IERC20(WKCS).balanceOf(address(this)).sub(initialBalance);
        if (newBalance > 2) {
    ...
    function swapTokensForWKCS(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = WKCS; 
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        IERC20(WKCS).approve(address(uniswapV2Router), ethAmount);
